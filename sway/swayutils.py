...
def command(cmd):
    j = _subprocess2json(["swaymsg", cmd])
    return j[0]["success"]


def get_workspaces():
    return _subprocess2json(["swaymsg", "-t", "get_workspaces"])


def new_workspace_num():
    workspaces = get_workspaces()
    sorted_workspaces = SortedList(workspace["num"]
...
def workspaces_for_current_output():
    ...
    return focused_workspace, list(
        filter(lambda x: x["output"] == focused_workspace["output"],
               workspaces))
